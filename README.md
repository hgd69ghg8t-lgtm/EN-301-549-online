...
docs/                Generated output, served by GitHub Pages. Never
                     hand-edited — re-run the build instead. Everything
                     here, including the minified content-hashed CSS/JS
                     under docs/assets/, is produced by the build; the
                     readable CSS/JS sources live in scripts/source/.
...
**Hand-edit:** everything under `content/`, `data/source-metadata.json`, `data/clause-summaries.json`, `data/content-ownership.json`, `data/performance-budgets.json`, `scripts/sitemap.json`, everything under `scripts/source/` (the readable CSS and JavaScript sources), `docs/assets/img/` (the AccessibleDocs logo and favicons), `README.md`, and everything under `docs-for-maintainers/`. `data/etsi-content-hashes.json` is the one exception: it's committed, but it's only ever written by `scripts/update_etsi_hashes.py` (see "Reproduced ETSI wording integrity" below), never by hand.

**Generated — never hand-edit:** every file directly under `docs/` (`docs/*.html`, the content-hashed `docs/assets/css/*.css`, `docs/assets/js/*.js`, `docs/search-index.<hash>.json`, `docs/search-suggestions.<hash>.json`, `docs/sitemap.xml`, `docs/robots.txt` — including `docs/accessibility-statement.html`, kept as a generated redirect to the About page so old links still work). They are committed to the repository (GitHub Pages serves straight from `docs/` with no build step of its own), but they are output, not input. If you edit a generated `docs/` file directly, the next `python3 scripts/build.py` run will silently overwrite your change — and separately, `git diff --exit-code -- docs/` after a rebuild (see below) will show your manual edit as a difference the moment anyone rebuilds, so it can't quietly become the "real" version of the page. `scripts/build.py` never reads any *generated* file: the only thing it reads under `docs/` is the committed source PDF's size — so nothing that happens to already-committed `docs/` output can feed back into the next build.

**Production assets are minified and content-hashed.** The readable sources in `scripts/source/` (`style.css`, `core.js`, `search.js`, `tables.js`, `reader-library.js`, `search-worker.js`) are minified at build time by `scripts/minify-assets.mjs` (terser + csso, mature minifiers pinned via `package-lock.json`) and written as `style.<hash>.css`, `core.<hash>.js`, etc., where `<hash>` is derived from the final production bytes. A filename therefore changes exactly when its bytes change — which lets any host cache the assets "forever" (see `docs-for-maintainers/performance.md`) while a change still rolls out atomically across every page — and a rebuild from unchanged source is byte-identical, filenames included. Stale hashed files are deleted on every build, and the build fails if a page references a missing asset or an unreferenced hashed asset lingers. No source maps are published: the readable source is in this repository.

The build needs the Python 3 standard library plus Node.js (for the minifier only — run `npm ci` once):
...
- The site search is entirely static: the build writes two content-hashed indexes (deterministic, so reproducible builds still hold) — the full `search-index.<hash>.json` (one entry per heading section, glossary term, and page intro, with body excerpts) fetched only on `search.html`, where a Web Worker (`scripts/source/search-worker.js`) ranks it off the main thread with a synchronous fallback for browsers without workers; and the much smaller `search-suggestions.<hash>.json` (titles, clause numbers and page names only) that powers header autocomplete everywhere. No search service, no third-party library. Nothing to maintain by hand — both indexes regenerate from content on every build.
...
- **Persistence and early application:** the choice is stored with the other reader preferences in `localStorage` and applied by a synchronous inline script deliberately placed before the stylesheet in the page head — which prevents (or at worst minimises) a wrong-theme flash, and also keeps the browser-chrome `theme-color` in step with the *resolved* theme. The emitted script is minified (it ships on every page); its full rationale lives as the comment above `THEME_SCRIPT` in `scripts/build.py`. Malformed storage can never break rendering: the read sits in a try/catch and only the literal values "dark"/"light" are accepted. Unit tests assert the script's ordering, synchronous application and storage-safety statically; actual flash behaviour is still worth an occasional manual look in real browsers. Each preference is validated independently on load, so malformed or pre-dark-mode stored data never discards the others.
...
- **Token-level only:** the dark palette redefines the custom properties in `scripts/source/style.css` (in two deliberately identical blocks — one for the media query, one for the explicit attribute), and fixed white-text surfaces (the blue title band, navigation highlights, buttons) keep their colours in both themes.
...
npm run test:modules   # optional-module failure paths: every lazy bundle and search resource deliberately broken in turn (requires a Chromium install)
npm run test:performance  # performance budgets from data/performance-budgets.json, measured on a throttled mobile profile against the local generated site (requires a Chromium install)
```

`npm test` is the non-browser validation — pure Python + Node, and what should run on every commit. The Playwright suites (`test:a11y`, `test:layout`, `test:search`, `test:theme`, `test:modules`, `test:performance`) additionally need a Chromium binary; run `npx playwright install --with-deps chromium` once before the first local run. CI runs all of them. Every browser suite fails on any unexpected page error, console error, unhandled promise rejection or failed same-origin request (`scripts/test-helpers.mjs`), so a runtime regression can't hide behind a passing behavioural assertion. Manual browser, screen-reader and Windows high-contrast checks are still required before claiming conformance — see `docs-for-maintainers/accessibility-testing.md`. The layout test checks, on representative pages at seven viewport widths (320-1920px): no page-level horizontal overflow, no sidebar/content overlap, the content column actually growing on wider screens, table wrappers only scrolling when the table's measured minimum width genuinely exceeds the space, and the mobile contents disclosure still working. See [`.github/workflows/ci.yml`](.github/workflows/ci.yml) for exactly what runs in CI and why these tools specifically — see the comment at the top of that file.
...
The site ships no webfonts: typography uses the reader's own system font stack, so nothing is downloaded and no font licence applies to the production output. (Earlier revisions self-hosted Overpass and Source Sans 3 under the [SIL Open Font License 1.1](https://openfontlicense.org/); those files remain in the repository's git history under their original licence terms.) Re-verify the licence of any font added in the future.
